    ...
    fun getKeyFrameIndex(stateTime: Float): Int {
        ...
        when (playMode) {
            ...
            Animation.PlayMode.LOOP_PINGPONG -> {
                ...
                if (frameNumber >= keyFrames.size) frameNumber =
                    keyFrames.size - 2 - (frameNumber - keyFrames.size)
            }
            Animation.PlayMode.LOOP_RANDOM -> TODO("Not implemetend")
            Animation.PlayMode.REVERSED -> frameNumber =
                Math.max(keyFrames.size - frameNumber - 1, 0)
...
class Aseprite(private val texture: Texture, val json: AsepriteJson) {
    ...
    private fun invalidSlice(name: String): Nothing =
        TODO("Invalid slice name $name. Other candidates : ${json.meta.slices.map { it.name }}")
    ...
    fun splitedIndex(
        index: Int,
        splitted: kotlin.Array<kotlin.Array<TextureRegion>>
    ): Pair<Int, Int> {
...
class AsepriteLoader(resoler: FileHandleResolver) :
    ...
    override fun getDependencies(
        ...
        result.add(
            AssetDescriptor(
                file?.pathWithoutExtension() + ".json",
