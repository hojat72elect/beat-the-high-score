...
class OptionsScreen(assetManager: AssetManager) : ScreenAdapter() {
            ...
            scrollPane {
                ...
                table {
                    ...
                    addSelect(bundle["options.credit"], { Config.credit = it }, { Config.credit }, mapOf(
                        bundle["options.credit.pixel"] to "pixel", bundle["options.credit.ghost"] to "ghost"
                    ), "sheets/transition2"
                    )
    ...
    private fun KTableWidget.addSelect(
        name: String, setter: (String) -> Unit, getter: () -> String, options: Map<String, String>, default: String
    ) {

        label(name, style = "option").cell(align = Align.left, fillX = true, expandX = true)
        val select = selectBoxOf(options.keys.asGdx(), style = "option").cell(align = Align.left)
