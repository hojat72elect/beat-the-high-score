...
object Config {
    ...
    private inline fun <reified T> pref(default: T) =
        ByPreference(preferences, T::class.java, default)
    ...
    class ByPreference<T>(
        ...
        operator fun getValue(thisRef: Any?, property: KProperty<*>): T {
            val result: Any = when (clazz) {
                String::class.javaObjectType -> preferences.getString(
                    property.name,
                    default as String
                )
                Boolean::class.javaObjectType -> preferences.getBoolean(
                    property.name,
                    default as Boolean
                )
        ...
        operator fun setValue(thisRef: Any?, property: KProperty<*>, value: T) {
            when (clazz) {
                String::class.javaObjectType -> preferences.putString(
                    property.name,
                    value as String
                )
                Boolean::class.javaObjectType -> preferences.putBoolean(
                    property.name,
                    value as Boolean
                )
